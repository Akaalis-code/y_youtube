...
def mthd_get_coef(para_the_func , para_n ,para_an_or_bn):
    if para_n == 0 and para_an_or_bn == 'an':
        a0 = (1/(math.pi)) * (integrate.quad(para_the_func , -(math.pi) , (math.pi)))
        return a0
    if para_n != 0 and para_an_or_bn == 'an':    
        an = (1/(math.pi)) * (integrate.quad((lambda x:para_the_func(x) * math.cos(para_n * x)) , -math.pi , math.pi))
        return an
    if para_n != 0 and para_an_or_bn == 'bn':    
        bn = (1/(math.pi)) * (integrate.quad((lambda x:para_the_func(x) * math.sin(para_n * x)) , -math.pi , math.pi))
        return bn

var_the_func = ?
for i in range (0,var_n) :
    if i == 0 :
        var_a0 = mthd_get_coef(var_the_func , i)
    else :
        var_lst_an = var_lst_an.append(mthd_get_coef(var_the_func , i , 'an'))
        var_lst_an = var_lst_an.append(mthd_get_coef(var_the_func , i , 'bn'))